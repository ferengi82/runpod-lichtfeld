...
RUN git clone --depth=1 --recurse-submodules --shallow-submodules "$LICHTFELD_REPO" LichtFeld-Studio && \
    ...
    python3 -c 'from pathlib import Path; import sys; p=Path("src/visualizer/CMakeLists.txt"); s=p.read_text(); old="target_link_libraries(lfs_visualizer PUBLIC lfs_rmlui)"; new="target_link_libraries(lfs_visualizer PUBLIC lfs_rmlui RmlUi::RmlUi)"; sys.exit("Expected Linux RmlUI link line not found") if old not in s and new not in s else None; p.write_text(s.replace(old,new)) if old in s else None' && \
